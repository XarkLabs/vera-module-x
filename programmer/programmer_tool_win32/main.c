...
int main(int argc, char *const argv[])
{
    ...
    while ((opt = getopt(argc, argv, "U:D:s:z:lp:#:")) != -1)
    {
        switch (opt)
        {
        // case 'F':
        ...
        //     break;
        // case 'i':
        //     fpga_image_index = atoi(optarg);
        //     break;
        // case 'I':
        //     filepath0 = optarg;
        //     break;
        case 'U':
            fileflashwrite = optarg;
            break;
        case 'D':
            fileflashdump = optarg;
            break;
        case 's':
        ...
        case 'l':
            ...
            break;
        case 'p':
            prog_devnum = atoi(optarg);
            break;
        case '#':
            prog_serial = strdup(optarg);
            break;
        // case 'B':
        //     fpga_boot_flash = true;
        //     break;
        // case '!':
        //     mass_erase = true;
        //     break;
    ...
    { // || !filepath) {
        ...
        fprintf(stderr, "  -l             List available USB programming devices\n");
        fprintf(stderr, "  -p <devnum>    Select device using device index, default first (shown using -l)\n");
        fprintf(stderr, "  -# <serial>    Select device using serial number (shown using -l)\n");
        fprintf(stderr, "\n");
        exit(1);
    }
    ...
    if (openFTDIDevice(prog_devnum, prog_serial) < 0)
    {
        fprintf(stderr, "No suitable FTDI device.  Try -l option.\n");

        return 0;
    }

    usb_init();
    spi_set_mode(SPI_MODE_FLASH); // only mode supported
